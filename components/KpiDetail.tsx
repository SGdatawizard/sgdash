...
export default function KpiDetail({ kpi }: { kpi: KpiWithData }) {
  ...
  const allCategories = useMemo(
    () => CATEGORIES.map((c) => computeCategory(kpi, c)),
    [kpi]
  );
  ...
  return (
    <div className="bg-paper rounded-sm shadow-card overflow-hidden">
      ...
      <div className="px-6 md:px-8 py-6">
        {computed.status === 'no-data' ? (
          <div className="flex flex-col items-center justify-center text-center py-16 gap-3">
            <div className="h-10 w-10 rounded-full border border-dashed border-ink/25" />
            <p className="text-sm text-ink/60 max-w-[320px]">
              No {category === 'Company' ? 'company-wide' : category} readings yet for this KPI.
              Add one from Data Entry to start the trend.
            </p>
          </div>
        ) : (
          <>
